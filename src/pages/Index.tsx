...
export default function Index() {
  ...
  const addMessage = useCallback((raw: string, type: IrcMessage["type"]) => {
    ...
    const msg: IrcMessage = { id: msgIdCounter++, raw, time, type, parsed };
  ...
  const sendRaw = useCallback((line: string) => {
  ...
  const connect = useCallback(() => {
    ...
    if (!inputRecovery.trim()) {
      addMessage("⚠ Введите RECOVERY код", "error");
      return;
    }
    ...
    const url = wsUrl || `wss://${settings.server}:${settings.port}/`;
    ...
    try {
      ...
      setStatus("error");
      addMessage(`⚠ Ошибка создания WebSocket: ${e}`, "error");
      return;
    }
    ...
    ws.onopen = () => {
      ...
      uptimeIntervalRef.current = setInterval(() => {
        setUptime(Math.floor((Date.now() - now.getTime()) / 1000));
      }, 1000);

      addMessage(`--- Соединение установлено (${url}) ---`, "system");
      ...
      sendLine(`IDENT ${settings.ident} -2 4030 1 2 :${settings.nick}`);
      sendLine(`HAAAPSI ${inputRecovery.trim()} 20 24`);
      sendLine(`RECOVER ${inputRecovery.trim()}`);
      sendLine(`DOMAINS ${settings.server}`);

      if (inputUser.trim() && inputPass.trim()) {
        sendLine(`REGISTER ${inputUser.trim()} ${inputPass.trim()} урарегнее`);
        sendLine(`USER ${inputUser.trim()} ${inputPass.trim()} урарегнее 05030c0a07`);
      }

      sendLine(`FWLISTVER 336`);
      sendLine(`ADDONS 252244 1`);
      sendLine(`MYADDONS 252244 1`);
      sendLine(`PHONE 960 1846 0 2 :chrome 148.0.0.0`);
      sendLine(`JOIN`);
    ...
    ws.onmessage = (evt) => {
      ...
      setBytesRecv((b) => b + text.length);
      const lines = text.split(/\r?\n/).filter(Boolean);
      lines.forEach((line) => {
        if (line.startsWith("PONG")) setPingMs(Date.now() - pingStartRef.current);
        addMessage(line, "in");
      });
    };
    ...
    ws.onclose = (evt) => {
      ...
      setStatus("offline");
      setConnectedAt(null);
      addMessage(`--- Соединение закрыто (код: ${evt.code}) ---`, "system");
    };
  }, [inputRecovery, inputUser, inputPass, settings, wsUrl, disconnect, addMessage]);
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div
              ...
              style={{ background: "linear-gradient(135deg, #00ffd5, #bf5fff)" }}
            >
              G
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            <span
              className={`w-2 h-2 rounded-full ${
                ...
                status === "error" ? "status-dot-offline" :
                "bg-muted-foreground"
              }`}
            />
          ...
          <div className="animate-fade-in max-w-md mx-auto">
            ...
            <div className="rounded-2xl border neon-border p-6 glass space-y-4">
              <div>
                ...
                />
                <p className="text-[10px] text-muted-foreground mt-1">Уникальный код восстановления персонажа</p>
              </div>
              ...
              <div className="pt-2">
                {status === "online" || status === "connecting" ? (
                  ...
                  <button
                    onClick={() => { connect(); setTab("messages"); }}
            ...
            <div className="flex-1 glass rounded-2xl border border-border overflow-y-auto p-4 font-mono text-xs space-y-[2px]">
              {messages.length === 0 ? (
                ...
                messages.map((msg) => (
                  <div
                    key={msg.id}
                    className="flex gap-2 leading-relaxed hover:bg-white/5 rounded px-1 py-[1px] transition-colors group"
                  >
                    ...
                    <span className="break-all">
                      {msg.type === "in" && msg.parsed ? (
                        <>
                          <span style={{ color: getCommandColor(msg.parsed.command), fontWeight: 700 }}>
                            {msg.parsed.command}
                          </span>
                          {" "}
                        ...
                        <span style={{
                          color:
                            msg.type === "system" ? "#bf5fff" :
                            msg.type === "error" ? "#ff2e2e" :
                            msg.type === "out" ? "#666" : "#aaa"
                        }}>
                          {msg.raw}
                        </span>
                      )}
              ...
              <div
                ...
                style={{
                  ...
                  background: status === "online" ? "radial-gradient(circle, hsla(120,100%,40%,0.15), transparent)" :
                    ...
                    status === "error" ? "radial-gradient(circle, hsla(0,90%,60%,0.15), transparent)" :
                    "transparent",
                  boxShadow: status === "online" ? "0 0 30px hsla(120,100%,40%,0.3)" : "none",
                }}
              >
                <Icon
                  name={status === "online" ? "Wifi" : status === "connecting" ? "Loader2" : "WifiOff"}
                  size={36}
          ...
          <div className="animate-fade-in max-w-lg mx-auto">
            ...
            <div className="glass border neon-border rounded-2xl p-6 space-y-4 mb-4">
              {([
                ...
                { key: "port", label: "Порт", placeholder: "443" },
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="wss://galaxy.mobstudio.ru:443/"
                  className="w-full bg-muted border border-border rounded-xl px-4 py-3 font-mono text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-[#00ffd5] transition-colors"
                />
                <p className="text-[10px] text-muted-foreground mt-1">Если пусто — формируется из сервера и порта выше</p>
              </div>

              <button
                onClick={() => addMessage("--- Настройки сохранены ---", "system")}
